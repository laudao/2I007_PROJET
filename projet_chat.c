...
void PourKeyPress (XKeyPressedEvent *evmt) {
		...
		if (strcmp(keycode, "\n") == 0){
			...
			if (cury==260) {
				XClearWindow(dpy, wchat);
			}
			
			XDrawString(dpy,wchat, DefaultGC(dpy, ecran),cury,5,keycode,strlen(keycode));
			...
			XDrawString(dpy,wtextinput, DefaultGC(dpy, ecran),5,5,keycode,strlen(keycode));
		}
		keycode = NULL;
	}
}
